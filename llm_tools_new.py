...
#             dividend_yield=data['summaryDetail'].get('dividendYield', 'N/A'),
...
def build_prompts(comp_data, symbol):
    ...
    # Safely get financial metrics with default values
    pe_ratio_trailing = data['summaryDetail'].get('trailingPE', 'N/A')
    pe_ratio_forward = data['defaultKeyStatistics'].get('forwardPE', 'N/A')
    pb_ratio = data['defaultKeyStatistics'].get('priceToBook', 'N/A')
    ev_ebitda = data['defaultKeyStatistics'].get('enterpriseToEbitda', 'N/A')
    market_cap = data['summaryDetail'].get('marketCap', 'N/A')
    dividend_yield = data['summaryDetail'].get('dividendYield', 'N/A')

    # Safely calculate PEG ratio
    try:
        peg_ratio = pe_ratio_trailing / data['defaultKeyStatistics'].get('earningsQuarterlyGrowth', 1)
